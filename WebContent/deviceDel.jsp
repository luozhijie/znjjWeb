						...
						<tbody>
							<c:forEach items="${deviceList }" var="device">
								<tr id="tr${device.deviceId }">
									...
									<td><button id="del${device.deviceId }"
											value="${device.deviceId }" class="btn btn-danger">删除</button></td>
								</tr>
								<script>
									$("#del${device.deviceId }").click(function() {
										$.get("ActionServlet?stat=deviceDel&did="+$("#del${device.deviceId }").val(),function(data,state){
											if(data=="删除成功"){
												$("#tr${device.deviceId }").remove();
											}else{
												alert(data);
											}
										});
